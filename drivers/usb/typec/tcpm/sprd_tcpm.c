...
ATOMIC_NOTIFIER_HEAD(oplus_chg_usb_tcpm_event_notifier);
EXPORT_SYMBOL_GPL(oplus_chg_usb_tcpm_event_notifier);
int oplus_chg_usb_tcpm_reg_event_notifier(struct notifier_block *nb)
{
	return atomic_notifier_chain_register(&oplus_chg_usb_tcpm_event_notifier, nb);
}
EXPORT_SYMBOL_GPL(oplus_chg_usb_tcpm_reg_event_notifier);

void oplus_chg_usb_tcpm_unreg_event_notifier(struct notifier_block *nb)
{
	atomic_notifier_chain_unregister(&oplus_chg_usb_tcpm_event_notifier, nb);
}
EXPORT_SYMBOL_GPL(oplus_chg_usb_tcpm_unreg_event_notifier);
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (port->svid_header == 0) {
		port->svid_header = SPRD_PD_IDH_VID(vdo);
		atomic_notifier_call_chain(&oplus_chg_usb_tcpm_event_notifier, SPRD_CMD_DISCOVER_IDENT, &port->svid_header);
	}
#endif
}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
		if (pmdata->nsvids == 1) {
			port->svid_header = svid;
			atomic_notifier_call_chain(&oplus_chg_usb_tcpm_event_notifier, SPRD_CMD_DISCOVER_SVID, &port->svid_header);
		}
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	port->svid_header = 0;
	atomic_notifier_call_chain(&oplus_chg_usb_tcpm_event_notifier, SNK_UNATTACHED, &port->svid_header);
#endif
	port->pd_capable = false;
	sprd_tcpm_log(port, "%s:line%d pd_capable flase", __func__, __LINE__);
	port->pps_data.supported = false;
