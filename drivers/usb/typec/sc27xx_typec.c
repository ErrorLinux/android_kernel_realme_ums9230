...
ATOMIC_NOTIFIER_HEAD(oplus_chg_typec_event_notifier);
EXPORT_SYMBOL_GPL(oplus_chg_typec_event_notifier);
int oplus_chg_typec_reg_event_notifier(struct notifier_block *nb)
{
	return atomic_notifier_chain_register(&oplus_chg_typec_event_notifier, nb);
}
EXPORT_SYMBOL_GPL(oplus_chg_typec_reg_event_notifier);

void oplus_chg_typec_unreg_event_notifier(struct notifier_block *nb)
{
	atomic_notifier_chain_unregister(&oplus_chg_typec_event_notifier, nb);
}
EXPORT_SYMBOL_GPL(oplus_chg_typec_unreg_event_notifier);

static u32 usb_online;
bool oplus_chg_typec_get_usb_present(void)
{
	return usb_online;
}
EXPORT_SYMBOL_GPL(oplus_chg_typec_get_usb_present);
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
		usb_online = true;
		atomic_notifier_call_chain(&oplus_chg_typec_event_notifier, POWER_SUPPLY_PROP_ONLINE, &usb_online);
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
void usb_audio_on_set_cc_level_default(void)
{
        if (typec_sc == NULL)
		return;
        sc27xx_typec_set_rp_level_value(RP_80UA);
}
EXPORT_SYMBOL_GPL(usb_audio_on_set_cc_level_default);
#endif
